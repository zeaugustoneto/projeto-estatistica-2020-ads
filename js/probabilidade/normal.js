let botao_normal = document.getElementById("botao_normal")

let distribuicao_Normal = () => {
    let dp = document.getElementById('desvioPadrao').value
    ...
    if (condicao == "maiorQue") {
     ...
     elemento = tabelaZ[l][c]}
        ...
        if ((valores[0] > media && valores[1] > media) || (valores[0] < media && valores[1] < media)) {
            let z = ((valores[0] - media) / dp).toFixed(2)
            if (z < 0) { z = z * -1 }
           ...
           // let dois = tabelaZ[l][c]
            ...
            probabilidade = ((um + dois) * 100).toFixed(2)
            if (probabilidade < 0) probabilidade = probabilidade * -1
        }
...
botao_normal.addEventListener('click', distribuicao_Normal)